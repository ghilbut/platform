...
AWS Support, Trusted Advisor만 허용한다. 그 밖의 API 호출은 명시적으로 거부한다. 관리 계정의
opt-in 리전은 모두 비활성화하고, `account:EnableRegion`도 거부한다.

AWS는 기본 활성 리전(예: `us-east-1`)을 비활성화할 수 없다. 그러나 권한 경계가
`us-east-1`을 포함한 모든 리전에서 워크로드 API를 거부한다.
