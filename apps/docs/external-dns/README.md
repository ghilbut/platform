...
CPA Istio `public` Gateway의 `id.ghilbut.com` CNAME과 TXT ownership record를 Route 53에서 관리한다. `external-dns` ServiceAccount는 `domains-cpa-external-dns` IAM 역할만 사용할 수 있다.

## 연결

- [Argo CD Application](../../argo-apps/external-dns.yaml)
- [external-dns manifest 디렉터리](../../argo-apps/external-dns/)
- [Domains external-dns OpenTofu module](../../../domains/tofu/modules/external-dns/)
