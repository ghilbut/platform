...
- `RULEBOOK.md`, `RUN-<APPLICATION>-YYYY-mm-dd-PLAN.md`, `RUNBOOK.md`, `PLAYBOOK.md`는 필요한 경우에만 만든다.
- `RULEBOOK.md`는 애플리케이션 문서 또는 운영 규칙을, 실행 계획은 `RUN-<APPLICATION>-YYYY-mm-dd-PLAN.md`를, `RUNBOOK.md`는 반복 가능한 운영 절차를, `PLAYBOOK.md`는 장애 복구·이전 같은 예외 절차를 기록한다.
- `RUNBOOK.md`와 실행 계획은 [Documentation RULEBOOK RUNBOOK](../../docs/RULEBOOK.md#runbook)을 따른다.
