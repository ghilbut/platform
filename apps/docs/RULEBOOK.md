...
- `RULEBOOK.md`, `RUNBOOK.md`, `PLAYBOOK.md`는 필요한 경우에만 만든다.
- `RULEBOOK.md`는 애플리케이션 문서 또는 운영 규칙을, `RUNBOOK.md`는 반복 가능한 운영 절차를, `PLAYBOOK.md`는 장애 복구·이전 같은 예외 절차를 기록한다.
- `README.md`의 연결은 디렉터리 단위로 기록한다. 개별 manifest나 같은 문서 디렉터리의 문서를 나열하지 않는다.
...
- 관련 Argo CD manifest, OpenTofu root, K3s 구성 디렉터리는 앱 `README.md`에서 상대 링크로 추적한다.
